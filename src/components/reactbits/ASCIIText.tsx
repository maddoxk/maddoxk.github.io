...
class CanvAscii {
  ...
  updateRotation() {
    const mapVal = (n: number, start: number, stop: number, start2: number, stop2: number) =>
      ((n - start) / (stop - start)) * (stop2 - start2) + start2;
    const x = mapVal(this.mouse.y, 0, this.height, 0.5, -0.5);
    const y = mapVal(this.mouse.x, 0, this.width, -0.5, 0.5);
